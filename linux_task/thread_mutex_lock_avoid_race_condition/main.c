...
void main()
{
	...
	int ret_val=0;
	pthread_t thread_id[NUMBER_OF_THRED]={0};
	...
	for(int i=0;i<NUMBER_OF_THRED;i++)
	{
		...
		ret_val = pthread_create(thread_id+i,&thread_attr, (i%2?(*(ptr+1)):(*ptr)) ,(void *)NULL);		//using Default Arguments and changed attributes
		if(0 != ret_val)
		{
